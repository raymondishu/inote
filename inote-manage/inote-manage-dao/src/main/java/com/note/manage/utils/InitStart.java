...
public class InitStart {
	...
	private void InitSystem() {
		try {
			...
			p1.load(ips1);
			String hbase_zookeeper_quorum = p1
					.getProperty("hbase_zookeeper_quorum");// 地址
			String hbase_zookeeper_property_clientPort = p1
					.getProperty("hbase_zookeeper_property_clientPort");// 端口
			int hbase_pool_size = Integer.parseInt(p1
					.getProperty("hbase_pool_size"));// 连接池
			Constants.HBASE_WRITE_BUFFER = Long.parseLong(p1
					.getProperty("hbaseWriteBuffer"));// 文件检索目录
			Constants.CONFIG = HBaseConfiguration.create();// 创建配置信息类
			Constants.CONFIG.set("hbase.zookeeper.quorum",
					hbase_zookeeper_quorum);// zookeeper的地址
			Constants.CONFIG.set("hbase.zookeeper.property.clientPort",
					hbase_zookeeper_property_clientPort);// zookeeper的端口
			Constants.POOL = new HTablePool(Constants.CONFIG, hbase_pool_size);// 创建连接池
			...
			hColumnDescriptor.setMaxVersions(5);// 设置表中数据的最大版本，如果只需要保存最新版本的数据，那么可以设置setMaxVersions(1)
			...
			int redis_timeout = Integer.parseInt(p3
					.getProperty("redis_timeout"));
