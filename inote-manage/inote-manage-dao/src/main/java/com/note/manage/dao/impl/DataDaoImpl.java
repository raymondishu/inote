package com.note.manage.dao.impl;
...
import com.note.manage.utils.JsonUtil;
...
public class DataDaoImpl implements DataDao {
	...
	public boolean insertData(String tableName, String rowKey, String family,
		...
		try {
			table = Constants.POOL.getTable(tableName);
	...
	public boolean insertData(String tableName, String rowKey, String[][] famQuaVals)  {
		...
		try {
			table = Constants.POOL.getTable(tableName);
			table.setAutoFlush(false);//AutoFlush指的是在每次调用HBase的Put操作，是否提交到HBase Server。 默认是true,每次会提交。如果此时是单条插入，就会有更多的IO,从而降低性能
	...
	public ResultScanner queryAll(String tableName) throws IOException {
		HTableInterface table = Constants.POOL.getTable(tableName);// 从连接池中获取表
	...
	public List<Note> queryNoteListByRowKey(String rowKey)
		...
		try {
			HTableInterface table = Constants.POOL.getTable(Constants.NOTEBOOK_TABLE_NAME);// 从连接池中获取表
	...
	public Note queryNoteByRowKey(String rowKey)
	{
		...
		Note note =null;
		try {
			HTableInterface table = Constants.POOL.getTable(Constants.NOTE_TABLE_NAME);// 从连接池中获取表
			Get get = new Get(rowKey.getBytes());// 获取get，注入rowKey
			row = table.get(get);
			if (row!=null) {
				note = new Note();
	...
	public ResultScanner queryByReg(String tableName, String reg)
		...
		HTableInterface table = Constants.POOL.getTable(tableName);// 从连接池中获取表
		Scan scan = new Scan();// 创建scan，用于查询
		scan.addColumn(Bytes.toBytes(Constants.NOTEBOOK_FAMLIY_NOTEBOOKINFO), Bytes.toBytes(Constants.NOTEBOOK_NOTEBOOKINFO_CLU_NOTEBOOKNAME));
	...
	public void copyData(String fromTableName, String toTableName, String fromRowKey, String toRowKey) throws IOException {
		HTableInterface fromTable = Constants.POOL.getTable(fromTableName);// 从连接池中获取表
		HTableInterface toTable = Constants.POOL.getTable(toTableName);//从连接池中获取表
		Get get = new Get(fromRowKey.getBytes());// 获取get，注入rowKey
		Result row = fromTable.get(get);// 获取单条row
		toTable.setAutoFlush(false);//AutoFlush指的是在每次调用HBase的Put操作，是否提交到HBase Server。 默认是true,每次会提交。如果此时是单条插入，就会有更多的IO,从而降低性能
